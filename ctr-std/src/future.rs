...
///
/// Panics if no task has been set or if the waker has already been
    ...
    F: FnOnce(&LocalWaker) -> R
{
    let waker_ptr = TLS_WAKER.with(|tls_waker| {
        // Clear the entry so that nested `with_get_waker` calls
...
pub fn poll_in_task_waker<F>(f: Pin<&mut F>) -> Poll<F::Output>
where
    F: Future
{
