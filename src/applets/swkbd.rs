...
impl Swkbd {
    ...
    pub fn get_bytes(&mut self, buf: &mut [u8]) -> Result<Button, Error> {
        unsafe {
            match swkbdInputText(self.state.as_mut(), buf.as_mut_ptr(), buf.len() as u32) {
                raw::SWKBD_BUTTON_NONE => Err(self.parse_swkbd_error()),
                raw::SWKBD_BUTTON_LEFT => Ok(Button::Left),
                raw::SWKBD_BUTTON_MIDDLE => Ok(Button::Middle),
                raw::SWKBD_BUTTON_RIGHT => Ok(Button::Right),
                _ => unreachable!(),
            }
        }
    }
    ...
    fn parse_swkbd_error(&self) -> Error {
        match self.state.result {
            raw::SWKBD_INVALID_INPUT => Error::InvalidInput,
            raw::SWKBD_OUTOFMEM => Error::OutOfMem,
            raw::SWKBD_HOMEPRESSED => Error::HomePressed,
            raw::SWKBD_RESETPRESSED => Error::ResetPressed,
            raw::SWKBD_POWERPRESSED => Error::PowerPressed,
            raw::SWKBD_PARENTAL_OK => Error::ParentalOk,
            raw::SWKBD_PARENTAL_FAIL => Error::ParentalFail,
            raw::SWKBD_BANNED_INPUT => Error::BannedInput,
            _ => unreachable!(),
        }
    }
}
