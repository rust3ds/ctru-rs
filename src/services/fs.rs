...
impl Fs {
    ...
    pub fn sdmc(&self) -> Result<Archive> {
        unsafe {
            ...
            let path = raw::fsMakePath(PathType::Empty.into(), ptr::null() as _);
            let r = raw::FSUSER_OpenArchive(&mut handle, id.into(), path);
            if r < 0 {
                Err(Error::from(r))
            } else {
                Ok(Archive {
...
impl File {
    ...
    pub fn set_len(&mut self, size: u64) -> IoResult<()> {
        unsafe {
            let r = raw::FSFILE_SetSize(self.handle, size);
            if r < 0 {
                Err(IoError::new(IoErrorKind::PermissionDenied, r.to_string()))
            } else {
                Ok(())
            }
        }
    ...
    pub fn metadata(&self) -> IoResult<Metadata> {
        ...
        unsafe {
            let mut size = 0;
            let r = raw::FSFILE_GetSize(self.handle, &mut size);
            if r < 0 {
                Err(IoError::new(IoErrorKind::PermissionDenied, r.to_string()))
            } else {
    ...
    fn read(&mut self, buf: &mut [u8]) -> IoResult<usize> {
        unsafe {
            ...
            if r < 0 {
                Err(IoError::new(IoErrorKind::Other, r.to_string()))
            } else {
                Ok(n_read as usize)
            }
        }
...
impl OpenOptions {
    ...
    fn _open(&self, path: &Path, flags: FsOpen) -> IoResult<File> {
        unsafe {
            ...
            let fs_path = raw::fsMakePath(PathType::UTF16.into(), path.as_ptr() as _);
            let r = raw::FSUSER_OpenFile(&mut file_handle, self.arch_handle,
...
impl<'a> Iterator for ReadDir<'a> {
    ...
    fn next(&mut self) -> Option<IoResult<DirEntry<'a>>> {
        unsafe {
            ...
            let entry_count = 1;
            let r = raw::FSDIR_Read(self.handle.0, &mut entries_read,
                                          entry_count, &mut ret.entry);

            if r < 0 {
                return Some(Err(IoError::new(IoErrorKind::Other, r.to_string())))
            }
...
pub fn create_dir<P: AsRef<Path>>(arch: &Archive, path: P) -> IoResult<()> {
    unsafe {
        ...
        let fs_path = raw::fsMakePath(PathType::UTF16.into(), path.as_ptr() as _);
        let r = raw::FSUSER_CreateDirectory(arch.handle, fs_path,
...
pub fn remove_dir<P: AsRef<Path>>(arch: &Archive, path: P) -> IoResult<()> {
    unsafe {
        let path = to_utf16(path.as_ref());
        let fs_path = raw::fsMakePath(PathType::UTF16.into(), path.as_ptr() as _);
        let r = raw::FSUSER_DeleteDirectory(arch.handle, fs_path);
        if r < 0 {
            Err(IoError::new(IoErrorKind::Other, r.to_string()))
        } else {
            Ok(())
...
pub fn remove_dir_all<P: AsRef<Path>>(arch: &Archive, path: P) -> IoResult<()> {
    unsafe {
        let path = to_utf16(path.as_ref());
        let fs_path = raw::fsMakePath(PathType::UTF16.into(), path.as_ptr() as _);
        let r = raw::FSUSER_DeleteDirectoryRecursively(arch.handle, fs_path);
        if r < 0 {
            Err(IoError::new(IoErrorKind::Other, r.to_string()))
        } else {
            Ok(())
...
pub fn read_dir<P: AsRef<Path>>(arch: &Archive, path: P) -> IoResult<ReadDir> {
    unsafe {
        ...
        let path = to_utf16(path.as_ref());
        let fs_path = raw::fsMakePath(PathType::UTF16.into(), path.as_ptr() as _);
        let r = raw::FSUSER_OpenDirectory(&mut handle, arch.handle, fs_path);
        if r < 0 {
            Err(IoError::new(IoErrorKind::Other, r.to_string()))
        } else {
...
pub fn remove_file<P: AsRef<Path>>(arch: &Archive, path: P) -> IoResult<()> {
    unsafe {
        let path = to_utf16(path.as_ref());
        let fs_path = raw::fsMakePath(PathType::UTF16.into(), path.as_ptr() as _);
        let r = raw::FSUSER_DeleteFile(arch.handle, fs_path);
        if r < 0 {
            Err(IoError::new(IoErrorKind::Other, r.to_string()))
        } else {
            Ok(())
...
pub fn rename<P, Q>(arch: &Archive, from: P, to: Q) -> IoResult<()>
    ...
    unsafe {
        ...
        let fs_from = raw::fsMakePath(PathType::UTF16.into(), from.as_ptr() as _);
        let fs_to = raw::fsMakePath(PathType::UTF16.into(), to.as_ptr() as _);

        let r = raw::FSUSER_RenameFile(arch.handle, fs_from, arch.handle, fs_to);
        if r == 0 {
            return Ok(())
        }
        let r = raw::FSUSER_RenameDirectory(arch.handle, fs_from, arch.handle, fs_to);
        if r == 0 {
            return Ok(())
        }
        Err(IoError::new(IoErrorKind::Other, r.to_string()))
...
impl Drop for Archive {
    fn drop(&mut self) {
        unsafe {
            raw::FSUSER_CloseArchive(self.handle);
        }
    }
}
...
impl Drop for Dir {
    fn drop(&mut self) {
        unsafe {
            raw::FSDIR_Close(self.0);
        }
    }
}

impl From<PathType> for raw::FS_PathType {
    fn from(p: PathType) -> Self {
        use self::PathType::*;
        match p {
            Invalid => raw::PATH_INVALID,
            Empty => raw::PATH_EMPTY,
            Binary => raw::PATH_BINARY,
            ASCII => raw::PATH_ASCII,
            UTF16 => raw::PATH_UTF16,
        }
    }
}

impl From<ArchiveID> for raw::FS_ArchiveID {
    fn from(a: ArchiveID) -> Self {
        use self::ArchiveID::*;
        match a {
            RomFS => raw::ARCHIVE_ROMFS,
            Savedata => raw::ARCHIVE_SAVEDATA,
            Extdata => raw::ARCHIVE_EXTDATA,
            SharedExtdata => raw::ARCHIVE_SHARED_EXTDATA,
            SystemSavedata => raw::ARCHIVE_SYSTEM_SAVEDATA,
            Sdmc => raw::ARCHIVE_SDMC,
            SdmcWriteOnly => raw::ARCHIVE_SDMC_WRITE_ONLY,
            BossExtdata => raw::ARCHIVE_BOSS_EXTDATA,
            CardSpiFS => raw::ARCHIVE_CARD_SPIFS,
            ExtDataAndBossExtdata => raw::ARCHIVE_EXTDATA_AND_BOSS_EXTDATA,
            SystemSaveData2 => raw::ARCHIVE_SYSTEM_SAVEDATA2,
            NandRW => raw::ARCHIVE_NAND_RW,
            NandRO => raw::ARCHIVE_NAND_RO,
            NandROWriteAccess => raw::ARCHIVE_NAND_RO_WRITE_ACCESS,
            SaveDataAndContent => raw::ARCHIVE_SAVEDATA_AND_CONTENT,
            SaveDataAndContent2 => raw::ARCHIVE_SAVEDATA_AND_CONTENT2,
            NandCtrFS => raw::ARCHIVE_NAND_CTR_FS,
            TwlPhoto => raw::ARCHIVE_TWL_PHOTO,
            NandTwlFS => raw::ARCHIVE_NAND_TWL_FS,
            GameCardSavedata => raw::ARCHIVE_GAMECARD_SAVEDATA,
            UserSavedata => raw::ARCHIVE_USER_SAVEDATA,
            DemoSavedata => raw::ARCHIVE_DEMO_SAVEDATA,
        }
    }
}