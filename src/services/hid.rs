...
pub struct TouchPosition(raw::touchPosition);

/// Represents the current position of the 3DS circle pad.
pub struct CirclePosition(raw::circlePosition);
...
impl TouchPosition {
    ...
    pub fn new() -> Self {
        TouchPosition(raw::touchPosition { px: 0, py: 0 })
    }

    /// Returns the current touch position in pixels.
    pub fn get(&mut self) -> (u16, u16) {
        unsafe { raw::hidTouchRead(&mut self.0); }
        (self.0.px, self.0.py)
    }
}
...
impl CirclePosition {
    ...
    pub fn new() -> Self {
        CirclePosition(raw::circlePosition { dx: 0, dy: 0 })
    }

    /// Returns the current circle pad position in (x, y) form.
    pub fn get(&mut self) -> (i16, i16) {
        unsafe { raw::hidCircleRead(&mut self.0); }
        (self.0.dx, self.0.dy)
    }
}
